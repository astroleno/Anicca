...
Corrective work makes success and failure artifacts mutually exclusive. A failed visual run publishes `artifacts/visual-smoke/dialogue-failure` with the head SHA, run ID and attempt, current and last successful step, full error stack, step history, incremental screenshots, DOM snapshot, renderer/page state and server output. It also makes a best-effort attempt to capture the failure moment; if the screenshot protocol itself fails, the manifest records `screenshotError` instead of claiming a `failure-screenshot.png` exists. It never falls back to the tracked `artifacts/visual-smoke/dialogue` directory. Each viewport, interaction scenario and Playwright condition wait emits a structured marker.
