...
export function InteractiveNebulaShader({
  ...
  useEffect(() => {
    ...
    if (mat) {
      ...
      mat.uniforms.themeColor.value.set(color.r, color.g, color.b);
    }
  }, [hasActiveReminders, hasUpcomingReminders, disableCenterDimming, themeHex]);
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void mainImage(out vec4 O, in vec2 fragCoord) {
        // Centered, aspect-corrected UV: origin at screen center
        ...
        for (int i = 0; i <= 5; i++) {
          ...
          // Theme-tinted palette (driven by themeColor)
          float intensity = f;
          vec3 base = hasActiveReminders
            ? themeColor * (0.6 + 4.0 * intensity)
            : hasUpcomingReminders
            ? themeColor * (0.5 + 3.0 * intensity)
            : themeColor * (0.4 + 2.5 * intensity);
    ...
    const uniforms = {
      ...
      themeColor:           { value: new THREE.Vector3() },
    };

    const material = new THREE.ShaderMaterial({ vertexShader, fragmentShader, uniforms });
    materialRef.current = material;
    // initialize theme color once
    const initial = new THREE.Color(themeHex);
    material.uniforms.themeColor.value.set(initial.r, initial.g, initial.b);
    const mesh = new THREE.Mesh(new THREE.PlaneGeometry(2, 2), material);
    scene.add(mesh);

    // Resize & mouse
    const onResize = () => {
      // Prefer container size; fall back to viewport if container not yet laid out
  ...
  return (
    <div
      ref={containerRef}
      className={`fixed inset-0 w-screen h-screen bg-background ${className}`}
